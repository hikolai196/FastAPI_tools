...
def write_photo_excel(
    ...
    for row_idx, (name, png_path) in enumerate(image_entries, start=2):
        ...
        if not png_path.exists():
            logger.warning("Excel embed skipped; missing file %s", png_path)
            ws.cell(row=row_idx, column=2, value="(missing image file)")
            continue
        try:
            ...
            logger.warning("Excel embed failed for %s: %s", name, exc)
